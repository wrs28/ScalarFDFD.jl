...
mutable struct PML{DIM,SIDE,FIN}<:AbstractBL{DIM,SIDE}
    ...
    (pml::PML{DIM,SIDE,true})(x::Number) where {DIM,SIDE} = Σ(x,pml.Δ,pml.depth,DIM,SIDE)
    (pml::PML{1,SIDE,true})(x::Number,y::Number) where {SIDE} = Σ(x,pml.Δ,pml.depth,1,SIDE)
    (pml::PML{2,SIDE,true})(x::Number,y::Number) where {SIDE} = Σ(y,pml.Δ,pml.depth,2,SIDE)
...
mutable struct cPML{DIM,SIDE,FIN}<:AbstractBL{DIM,SIDE}
    ...
    (cpml::cPML{DIM,SIDE,true})(x::Number) where {DIM,SIDE} = cΣ(x,cpml.Δ,cpml.depth,DIM,SIDE)
    (cpml::cPML{1,SIDE,true})(x::Number,y::Number) where {SIDE} = cΣ(x,cpml.Δ,cpml.depth,1,SIDE)
    (cpml::cPML{2,SIDE,true})(x::Number,y::Number) where {SIDE} = cΣ(y,cpml.Δ,cpml.depth,2,SIDE)
...
cΣ(x,Δ,depth,DIM,SIDE) = conj(Σ(x,Δ,depth,DIM,SIDE))
...
"""
struct MatchedBC{DIM,SIDE,CS,FIN} <: AbstractBC{DIM,SIDE}
    ...
    MatchedBC{DIM,SIDE}() where {DIM,SIDE} = new{DIM,SIDE,Union{},false}()
    MatchedBC{DIM,SIDE,CS}() where {DIM,SIDE,CS} = new{DIM,SIDE,CS,false}()
    MatchedBC{DIM,SIDE}(outgoing_qns, incoming_qns; kwargs...) where {DIM,SIDE} = new{DIM,SIDE,Union{},false}(outgoing_qns, incoming_qns)
    MatchedBC{DIM,SIDE,CS}(outgoing_qns, incoming_qns; kwargs...) where {DIM,SIDE,CS} = new{DIM,SIDE,CS,false}(outgoing_qns, incoming_qns)
    ...
    function MatchedBC{1,2,Polar}(outgoing_qns, incoming_qns, N; kwargs...)
        ...
        return new{1,2,Polar,true}(outgoing_qns, incoming_qns, Tuple(N), Tuple(dx), Tuple(xmin), xmax, i1, j1, i2, j2, v, M, direction)
    end

    (mbc::MatchedBC{DIM,SIDE,_1,false})(N, coordinate_system::Type{CS}, args...; kwargs...) where {DIM,SIDE,_1,CS} = MatchedBC{DIM,SIDE,CS}(mbc.outgoing_qns,mbc.incoming_qns,N; kwargs...)
    (mbc::MatchedBC{DIM,SIDE,CS,false})(N; kwargs...) where {DIM,SIDE,CS<:CoordinateSystem} = MatchedBC{DIM,SIDE,CS}(mbc.outgoing_qns,mbc.incoming_qns,N; kwargs...)
